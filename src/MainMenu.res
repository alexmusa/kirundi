...
@react.component
let make = (~onStart, ~onLessonSelect: int => unit, ~onSettings, ~isStarted) => {
  let (openGroup, setOpenGroup) = React.useState(_ => None)

  // Define the lesson ranges: (start, end)
  let lessonGroups = [
  ...
  <div className="flex flex-col min-h-screen max-w-4xl mx-auto font-sans bg-gray-50">
    /* Hero Section (One full screen height) */
